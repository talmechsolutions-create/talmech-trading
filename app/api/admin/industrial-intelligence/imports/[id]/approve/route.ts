...
import { approveIndustrialImportBatch } from '@/lib/industrial/importService';
...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    const batch = await approveIndustrialImportBatch(params.id, access.actor.username);
    return apiOk({ batch }, { headers: industrialNoStoreHeaders });
  } catch (error) {
    if (error instanceof Error) return apiError(error.message, 'Unable to approve import batch.', 400);
